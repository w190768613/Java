...
public class FileHandlerImpl implements FileHandler {
	...
	public StockInfo[] getStockInfoFromFile(String filePath) {
		...
		for (int i = 0; i < 61; i++)
			arrs[i] = new StockInfo(); // 初始化StockInfo类数组arrs
 ...
 // 读取interest.txt文件
	public UserInterest[] getUserInterestFromFile(String filePath) {
		UserInterest[] arrs = new UserInterest[500]; 
		String[] ifread = new String[60];
		for (int i = 0; i < arrs.length; i++)
			arrs[i] = new UserInterest(); // 初始化UserInterest数组arrs
		...
		try {
			...
			if (file.isFile() && file.exists()) { // 判断文件是否存在
				...
				while ((line = reader.readLine()) != null) {
					i++;
					ifread = line.split("");
					arrs[i].mark = new String[60];
					for (int j = 0; j < 60; j++)
						arrs[i].mark[j] = ifread[j];
				}
				reader.close();
			} else {
				System.out.println("找不到指定的文件");
